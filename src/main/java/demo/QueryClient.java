...
import rxnetty.EchoServer;
import rxnetty.ExamplesEnvironment;

/**
 * A TCP "Hello World" example that sends a string "Hello World!" to the target server and expects
 * an echo response.
 *
 ...
 * <h2>Default</h2>
 *
 * The default way is to just run this class with no arguments, which will start a server ({@link
 * EchoServer}) on an ephemeral port, send a "Hello World!" message to the server and print the
 * response.
 *
 * <h2>After starting {@link EchoServer}</h2>
 *
 * If you want to see how {@link EchoServer} work, you can run {@link EchoServer} by yourself and
 * then pass the port on which the server started to this class as a program argument:
 *
 * <PRE> java io.reactivex.netty.examples.tcp.echo.EchoClient [server port] </PRE>
 *
 * <h2>Existing TCP server</h2>
 *
 * You can also use this client to connect to an already running TCP server (different than {@link
 * EchoServer}) by passing the port and host of the existing server similar to the case above:
 *
 * <PRE> java io.reactivex.netty.examples.tcp.echo.EchoClient [server port] [server host] </PRE> If
 * the server host is omitted from the above, it defaults to "127.0.0.1"
 *
...
public final class QueryClient {
  ...
  public static void main(String[] args) {
    ...
    ExamplesEnvironment env = ExamplesEnvironment.newEnvironment(QueryClient.class);
...
//    SocketAddress serverAddress = env.getServerAddress(EchoServer.class, args);
    ...
    TcpClient.newClient("127.0.0.1", 11001)
